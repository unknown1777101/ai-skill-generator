...
- **Auto-activation**: Antigravity agents load this skill when you prompt with words like: *"design a new skill for...", "create a skill for...", "audit skill X"*
- **Manual invocation**: You can prompt the agent to explicitly use it:
  > "Use `skill-creator` to design a new skill for [task]"

### Path Selection:
When generating a new skill, the agent is required to ask where to place the output files:
1. **Current Project Folder**: Saves locally in `.agents/skills/<category>/<skill-name>/` (or `.agents/skills/<skill-name>/`).
2. **Global Antigravity Config**: Saves globally in `~/.gemini/config/skills/<category>/<skill-name>/`.
...
Every skill generated should be verified using the validation commands:
```bash
# Validate a single skill folder
skill-validate path/to/skill-dir

# Validate a full plugin package
plugin-validate path/to/plugin-dir
```
